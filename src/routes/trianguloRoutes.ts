...
class TrianguloRoutes {
    ...
    private totalAreas = async (req: Request, res: Response) => {
        await db.conectarBD() 
        //creamos un documento de la base de datos pero solo con los campos que queremos 
        type docTriangulo = {
            nombre: String,
            area: Number
        }
        //declaramos el esquema para que luego almacene ahi los resultados 
        let esquema: docTriangulo
        //el array para que almacene todos los documentos que se generen en la busqueda
        let array: Array<any> = new Array()
        let triangulos: any 
        const query: any =  await Triangulos.find()
        for (triangulos of query){   
        const triangulo: Triangulo = new Triangulo(query._nombre, query._base, query._lado2, query._lado3)
            triangulo.altura = query._altura 
            esquema = {
                nombre: query._nombre,
                area: triangulo.area()
            }
            array.push(esquema)
            console.log(`nombre ${triangulo.nombre} area: ${triangulo.area()}`)
        }
        console.log(array)
        res.json(array)
        await db.desconectarBD()
    ...
    misRutas(){
        ...
        this._router.get('/areas', this.totalAreas)
    }
}
